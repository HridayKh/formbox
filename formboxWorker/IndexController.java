public class IndexController {
	...
	public String submission(@PathVariable UUID formId, @RequestParam Map<String, String> payload,
		...
		try {
			...
			log.debug("Step 1 (Get Form - FormNotFound) took {} ms",
		...
		if (isHoneypot) {
			...
			log.debug("Step 5 (Honeypot caught & saved) took {} ms",
					System.currentTimeMillis() - stepStart);
		...
		if (turnstileFailed) {
			submissionService.asyncSaveSubmission(form.id(), request.getRemoteAddr(), payload, true);
			log.debug("Step 6 (Turnstile failed & saved) took {} ms",
					System.currentTimeMillis() - stepStart);
			return "submit/thanks";
		}
		...
		if (!form.allowFiles()) {
			try {
				...
				if (parts != null && !parts.isEmpty()) {
					log.debug("Step 7 (File check - forbidden) took {} ms",
							System.currentTimeMillis() - stepStart);
		...
		log.debug("Steps 13 & 14 (Async upload & webhook init) took {} ms",
				System.currentTimeMillis() - stepStart);
